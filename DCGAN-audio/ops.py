...
def fully_connected(input_, output_size, scope=None, stddev=0.1, with_bias = True, missing_dim=-1):
    shape = input_.get_shape().as_list()
    if missing_dim < 0: missing_dim = shape[-1]
    with tf.variable_scope(scope or "FC"):
        matrix = tf.get_variable("Matrix", [missing_dim, output_size], tf.float32,
            tf.random_normal_initializer(stddev=stddev))
...
def binary_cross_entropy_with_logits(logits, targets, name=None):
    ...
    with tf.name_scope(name, "bce_loss", [logits, targets]) as name:
